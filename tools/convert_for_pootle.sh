...
for d in $TRANSLATIONS_DIR/*/ ; do
    ...
    componentName=`basename "$d" | sed -E 's/jolla-cesky\.(.*)ts/\1/g'`
    ...
    for templatePath in $(find $TEMPLATES_DIR -type f -name "$componentName.ts"); do
        ...
        mkdir -p $outpath
        rm -rf $outpath/*
        ./transifex_to_pootle_converter.py $d/$TRANSIFEX_LANG.ts $templatePath $outpath/$componentName.ts $TARGET_LANG $TRANSLATION_MEMORY_PATH/$d/$TRANSLATION_MEMORY_LANG.ts
        retval=$?
        if [ $retval -ne 0 ]; then
            echo "./transifex_to_pootle_converter.py $d/$TRANSIFEX_LANG.ts $templatePath $outpath/$componentName.ts $TARGET_LANG $TRANSLATION_MEMORY_PATH/$d/$TRANSLATION_MEMORY_LANGts failed"
            break
        fi
    done
    if [ $retval -ne 0 ]; then
